...
async function getNewsData(query: string, page: number, pageSize: number) {
  const res = await fetch(`http://localhost:1337/api/blog-publicities?populate=*&pagination[page]=${page}&pagination[pageSize]=${pageSize}&filters[title][$contains]=${query}&sort=publishedAt:desc`, { next: { revalidate: 60 } })
  if (!res.ok) {
